...
1. Call \`vault0_task-list(status: "in_review")\` to find tasks awaiting approval.
...
4. If there is reasonable evidence of correlation → call \`vault0_task-complete(id: "<task-id>", solution: "<commit details>")\`
   - Example: \`vault0_task-complete(id: "01JA...", solution: "Completed in abc123 — added vault0_task-view to git agent permissions")\`
5. If no evidence of correlation → leave the task in \`in_review\`.
6. Report approved tasks (ID + title) in the commit summary.

**Note:** Only the Git Agent has access to \`vault0_task-complete\`. This is the exclusive mechanism for moving tasks to \`done\`.

If no tasks are in review, skip silently — do not report "no tasks to approve".

## The \`vault0_task-move\` tool does NOT support \`done\` status.

Only \`vault0_task-complete\` can transition to \`done\`.
`
