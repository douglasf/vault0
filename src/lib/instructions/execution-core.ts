...
1. **Read**: \`vault0_task-view(id)\` — mandatory fresh query even if you saw the task before.
...
5. **Submit**: \`vault0_task-move(id, status: "in_review")\`. **CRITICAL**: Never move directly to \`done\`.
...
If a commit occurs: stop immediately. Do not pick next task or call \`vault0_task-list\`.
`
